...
$(document).ready(function() {
  ...
  $('#container').delegate('#checkout','click',function() {
    ...
    $('#lowercontent').hide();
    $('#lowerportion').hide();
    $('#container').html("thanku for shopping").css({'font-size':'24','text-align':'center','padding-top':'30'});
  });   
});
function displayCart(item){ 
  ...
  for(var i = 0; i < jsonArray[item].length; i++) {
      ...
      $(newDiv).removeClass('hidden').attr({'id':i+"_"+item,'class':'myDiv'}).find('#myName').text(" name:" + jsonArray[item][i].name).siblings('#mycontent').text(" content: " + jsonArray[item][i].description).siblings('#myPrice').text(" Price: Rs." + jsonArray[item][i].price);           
...
function addToCart(myId){
  ...
  if(myRegex.test(quantity)){
    ...
    $('#'+ myId).find('input').val("");
    if(!bool) {
      $('#tableid').append('<tr id="row' +myId +'"><td>' + productName + '</td><td>' + price + '</td><td>' + quantity + '</td><td>' + subTotal + '</td><td><input type="submit" value="remove" class="remove" id="remove-' + myId + '"></td></tr>');
    }
  calculateTotal();
